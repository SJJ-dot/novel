package sjj.novel.view.refreshprogressbar;
...
public class RefreshProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (secondDurProgress > 0 && secondMaxProgress > 0) {
            ...
            secondRect.left = getMeasuredWidth() / 2 - tempW / 2;
            secondRect.right = getMeasuredWidth() / 2 + tempW / 2;
            secondRect.bottom = getMeasuredHeight();
            canvas.drawRect(secondRect, paint);
        }

        if (durProgress > 0 && maxProgress > 0) {
            paint.setColor(fontColor);
            fontRectF.right = getMeasuredWidth() * 1.0f * (durProgress * 1.0f / maxProgress);
            fontRectF.bottom = getMeasuredHeight();
